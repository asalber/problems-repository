...
1. Compute the probability of arriving more than `r patients} patients in `r hours} hours.
2. If the clinic has `r employees} physiotherapists and each of them can treat one patient per hour, what is the probability that a day there was some hour in which some patient can not be attended?
How many physiotherapists must be in the clinic to guarantee that this probability is less than 10\%?

## Solution

1. Let $X$ be the number of patients that arrive in `r hours} hours. 
$X\sim P(`r lambda1})$ and $P(X>`r patients})=`r round(sol1,4)`$.
2. Let $Y$ be the number of hours in a day in which some patient can not be treated.
