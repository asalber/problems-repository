...
## DATA GENERATION
x <- c(1, 2, 3, 4, 5, 6, 9, 12, 18, 24)
y <- c(245.61, 201.1, 164.64, 134.8, 110.36, 90.36, 49.59, 27.21, 8.2, 2.47)
z <- 0.85 * y
sumx <- sum(x)
sumy <- sum(y)
sumx2 <- sum(x^2)
...
mxy <- lm(x ~ y)
# Regression line x on z
mxz <- lm(x ~ z)
# Determination coeff
r2xy <- sxy^2/(s2x*s2y)
# Prediction x
xy150 <- mxy$coefficients[1] + mxy$coefficients[2] * 150
# Prediction y
yx10 <- myx$coefficients[1] + myx$coefficients[2] * 10
# Prediction x
xz150 <- mxz$coefficients[1] + mxz$coefficients[2] * 150
```
