...
\begin{solution}
\begin{enumerate}
\item $s_{xy}=\Sexpr{round(sxy, 4)}$ meters$\cdot$persons. That means that there is a direct linear relation between the meters that exceed pollution limits and the people affected by pneumonia.
...
\item Exponential regression model: $y=e^{\Sexpr{round(expyx$coefficients[1],4)} +  \Sexpr{round(expyx$coefficients[2],4)}x}$, and $y(5)=\Sexpr{round(expyx$coefficients[1] + expyx$coefficients[1] * 5, 4)}$ persons.
