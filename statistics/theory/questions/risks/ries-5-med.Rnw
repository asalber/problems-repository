...
<<tikzsource, echo=FALSE, results = hide>>=
...
\begin{enumerate}
\item Construct the probability tree for the random experiment of drawing a random person from the population and measuring if he or she consumes cocaine and if he or she has a heart attack.
\item Compute the probability that a random person of the population does not consume cocaine and does not have a heart attack.
\item Are the events of consuming cocaine and having a heart attack dependent?
\item Compute the relative risk and the odds ratio of suffering a heart attack consuming cocaine. Which association measure is more suitable for this study? Interpret it.
\end{enumerate}
\end{question}
...
\begin{enumerate}
\item 
<<tree, echo = F, results=tex>>=
include_tikz(graf01, name = "graf", format = typ, markup = "tex", library = "positioning", width = "7cm")
@


\item $P(\overline C\cap \overline H)=\Sexpr{round(pnoCnoH, 4)}$.
\item The events are dependent as $P(C)=\Sexpr{pC}\neq P(C|H)=\Sexpr{pCsiH}$.
\item $RR(H)=\Sexpr{round(rr, 4)}$ and $OR(H)=\Sexpr{round(or, 4)}$. The odds ratio is more suitable as the study is retrospective. That means that the odds of having a heart attack is $\Sexpr{round(or, 4)}$ times greater if a person consumes cocaine.
\end{enumerate}
\end{solution}
